...
type MealPlan = {
  id: string; title: string; goal: string; diet: string;
  duration: number; content: string; created_at: string;
};

const GOAL_EMOJI: Record<string, string> = {
  balanced: "⚖️", weight_loss: "🔥", high_protein: "💪",
  anti_inflammatory: "🫀", energy: "⚡",
};
...
export default function SavedPlans({ plans, onDelete, onRename, onGenerate }: Props) {
  return (
    <div className="flex-1 overflow-y-auto">
      <div className="sticky top-0 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-700 px-6 py-4 flex items-center justify-between z-10">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-gray-900 dark:text-gray-100">Meal Plans</h2>
          {plans.length > 0 && (
      ...
      <div className="max-w-2xl mx-auto px-4 py-4 divide-y divide-gray-100 dark:divide-gray-700">
        {plans.length === 0 ? (
          ...
          plans.map((p) => <PlanItem key={p.id} plan={p} onDelete={onDelete} onRename={onRename} />)
        )}
      </div>
    </div>
  );
...
function PlanItem({ plan, onDelete, onRename }: {
  plan: MealPlan; onDelete: (id: string) => void; onRename: (id: string, title: string) => void;
}) {
  ...
  const handleRenameSave = async () => {
    if (!renameValue.trim() || renameValue.trim() === plan.title) { setRenaming(false); return; }
    setSaving(true);
    ...
    onRename(plan.id, renameValue.trim());
    setRenaming(false);
    setSaving(false);
  };
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <div className="min-w-0 flex-1">
            {renaming ? (
              ...
              <p className="text-sm font-medium text-gray-800 dark:text-gray-100 truncate">{plan.title}</p>
            )}
            <p className="text-[11px] text-gray-400 dark:text-gray-500">{plan.duration} days · {dietLabel}</p>
          </div>
        </div>
